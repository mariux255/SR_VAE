...
def plot_autoencoder_stats(
    ...
    canvas = np.zeros((32 * rows, columns * 32))
    for i in range(rows):
        for j in range(columns):
            idx = i % columns + rows * j
            canvas[i * 32:(i + 1) * 32, j * 32:(j + 1) * 32] = x[idx].reshape((32, 32))
    ...
    for i in range(rows):
        for j in range(columns):
            idx = i % columns + rows * j
            canvas[i * 32:(i + 1) * 32, j * 32:(j + 1) * 32] = x_hat[idx].reshape((32, 32))
...
def plot_samples(ax, x):
    ...
    x_grid = make_grid(x.view(-1, 1, 32, 32), nrow=nrow).permute(1, 2, 0)
    ax.imshow(x_grid)
    ax.axis('off')
